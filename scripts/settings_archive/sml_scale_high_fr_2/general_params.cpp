...
struct P {
	double sim_time = 1323200;//7000;//1200080;//8553860;//30000;//8460580;//8460580;//250000;//8460580;//8460580;//8485920;//250000;//1260120;//8485920;//8485920;//1440140;//1440140;//8553860;//131400;//8485920;//120000//29416*20;//60000*firing_bin;// sim run time in ms
	int timestep = 20; // timestep between movements. e.g., 20ms between each movement command
	int t = 0; // time
	static const int x_size = 36;//40;//36;//42;//30;//26;
	static const int y_size = 36;//40;//36;//42;//30;//26;
	static const int layer_size = x_size * y_size;
	static const int EC_LI_II_Multipolar_Pyramidal_Count = 1296;//1600;//1764;//900;
	static const int MEC_LII_Stellate_Count = 1296;//1600;//1764;//900;
	static const int EC_LII_Axo_Axonic_Count = 834;//706;//646;//834;//834;//1200;//646;//534;//588;//300;
	static const int MEC_LII_Basket_Count = 833;//705;//645;//833;//833;//1200;//645;//533;//588;//300;
	static const int EC_LII_Basket_Multipolar_Count = 833;//705;//645;//833;//833;//1200;//645;//533;//588;//300;
	static const int layer_size_in = EC_LII_Axo_Axonic_Count+MEC_LII_Basket_Count+EC_LII_Basket_Multipolar_Count;
	static const int CA1_Pyramidal_Count = 1296;//1600;//1764;//900;
	...
	double pos[2] = {-2.9,-2.2};//{-5.4,-4.7};//{28.5,16.75}; // virtual animal position tracker. starting position: {x,y}
	double bpos[2] = {-2.9,-2.2};//{-5.4,-4.7};//{28.5,16.75}; // bump position tracker
	...
	double base_ext = 132.5;//200;//150;//300; // baseline ext input speed level	
	double speed_signaling = 0.95;//.35;//.6;//.3;//2.0;//.3; // setting for use of a constant virtual animal speed
	double fast_to_slow_ratio = 0.38/0.62;
	double dir_to_grc_g_fast = 33.082*1.1*1.15;//1.19;//.81;//.7;//.36;//.6;//.7;//.7;//.55;//.43;//.2;
	double dir_to_grc_g_slow = dir_to_grc_g_fast*fast_to_slow_ratio;
	double grc_to_in_g_fast = 0.8244568522+((1.952544646-0.8244568522)*.35);//.15);//.25);//.5);//1.9;//5;//3;//4;//0.8244568522+((1.952544646-0.8244568522)*1);//.7);//.7);//1);//*.15);//0.15);
	double grc_to_in_g_slow = grc_to_in_g_fast*fast_to_slow_ratio;	
	double in_to_grc_g_fast = 0.6259622633+((1.786298271-0.6259622633)*.25);//.25);//0);//0.6259622633;//.5;//1.2;//.5;//.5;//0.6259622633+((1.786298271-0.6259622633)*1);//.5);//*0); // range is perhaps 0.6259622633 to 1.786298271
	...
	double move_increment = 0.001;//0.002;//0.01;//0.018;//0.007;//0.018;//0.006;//0.005;//0.024;//0.018;//0.005;//0.018; // amount to move in position each move command
	...
	float pc_to_grc_g_fast = 71.14*.2667;//.23;//0.145;//.19;
	float pc_to_grc_g_slow = pc_to_grc_g_fast*fast_to_slow_ratio;
	double pc_sig = 3;//4;//3;//7.0; // sigma symbol; width of the place feild
	double pc_level = 267;//230;//145;//400; // place cell firing level
	...
	double grid_pattern_rot = 0;//-15;//-33.75; // angle value for rotation of grid pattern in plot
	...
	// vector<int> cent_x{0, -10, 10}; 
    // vector<int> cent_y{0, -12, -12};
	// vector<int> cent_x{0, -10,  10}; 
    // vector<int> cent_y{0, -10, -10};
	// vector<int> cent_x{0, -8,  8}; 
    // vector<int> cent_y{0, -10, -10};
	// vector<int> cent_x{0, -8,  8, 8}; 
    // vector<int> cent_y{0, -12, 12, -12};
	// vector<int> cent_x{0, -18,  18}; 
    // vector<int> cent_y{0, -10, -10};    
	// vector<int> cent_x{0, -16,  16}; 
    // vector<int> cent_y{0, -10, -10}; 
	// vector<int> cent_x{0, -16,  16, 16, -16}; 
    // vector<int> cent_y{0, -10, -10, -10, 10};
	// vector<int> cent_x{0, -16, 16}; 
    // vector<int> cent_y{0, -10, -10};    
	// vector<int> cent_x{0}; 
    // vector<int> cent_y{0}; 
	// vector<int> cent_x{0, -18,  18, 18, -18}; 
    // vector<int> cent_y{0, -10, -10, 10,  10}; 
	// vector<int> cent_x{0, -8,  8,    6, -6};
	// vector<int> cent_y{0, -12, 12, -12, 12};
	// vector<int> cent_x{0, -18,  18, 18}; 
    // vector<int> cent_y{0, -10, -10, 10}; 
	// vector<int> cent_x{0, -18, 18}; 
    // vector<int> cent_y{0, -10, 10}; 
	// vector<int> cent_x{0, -18, -18}; 
    // vector<int> cent_y{0, -10,  10}; 
	// vector<int> cent_x{0, -18, 18, -18, 0}; 
    // vector<int> cent_y{0, -10, 10,  10, 18}; 
	vector<int> cent_x{0, -18, 18, 0}; 
    vector<int> cent_y{0, -10, 10, 18}; 
	// vector<int> cent_x{0, -18,  18, 18, -18}; 
    // vector<int> cent_y{0, -10, -10, 10,  10};
	// vector<int> cent_x{0, -18, 18, -18, 18, 0}; 
    // vector<int> cent_y{0, -10, -10,  10, 10, 18}; 
